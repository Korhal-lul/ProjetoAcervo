...
public class Login implements Initializable {
		...
		buttonEntrar.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (lenUsuario == 0 || lenSenha == 0) {
					...
					String b = "sua senha";
					String c = a + " e " + b;
					String s = "Por favor, insira "
							+ (lenUsuario == 0 && lenSenha == 0 ? c : (lenUsuario > 0 && lenSenha == 0 ? b : a)) + ".";
						...
						if (textFieldUsuario.getText().equals(usuarios.get(i).getNome())) {
							...
							if (textFieldSenha.getText().equals(usuarios.get(i).getSenha())) {
								...
								try {
									...
									stage.getIcons()
											.add(new Image(this.getClass().getResource("/icons/logo.png").toString()));
									stage.show();
	...
	/*
	 * Nome da Função: setUsuario 
	 * Retorno: Void 
	 * Objetivo: Define o usuario logado na sessao:
	 */
	...
	/*
	 * Nome da Função: setEventHandler
	 *  Retorno: Void 
	 *  Objetivo:Evento para a tecla 'Enter' servir para pressionar o botao 'Entrar':
	 */
